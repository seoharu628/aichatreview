...
export async function onRequestPost({request,env}){
  ...
  let list=[];
  try{list=await load(env);}catch(e){
  ...
  if(b.action==='add'){
    ...
    if(list.some(r=>r.browserToken===bt&&r.pid===pid)){
      return json({error:'이 플랫폼에는 이미 리뷰를 작성했어요. 기존 리뷰를 수정해 주세요.'},409);
    }
  ...
  if(b.action==='edit'){
    ...
    if(r.sec!==String(b.sec||''))return json({error:'수정할 권한이 없어요.'},403);
    const rating=Math.round(Number(b.rating));
    if(!Number.isInteger(rating)||rating<1||rating>5){
      return json({error:'별점을 선택해 주세요.'},400);
    }
  ...
  if(b.action==='ownerDelete'){
    const r=list.find(x=>x.id===b.id);
    if(!r)return json({error:'리뷰가 없어요.'},404);
    if(r.sec!==String(b.sec||''))return json({error:'이 리뷰를 삭제할 권한이 없어요.'},403);
  ...
  if(b.action==='report'){
    ...
    r.rp.push({reason:String(b.reason||'기타').slice(0,200),ts:Date.now()});
    await save(env,list);
    return json({ok:true});
  }

  if(b.action==='login'){
    return json({ok:!!env.ADMIN_PW&&String(b.pw||'')===String(env.ADMIN_PW)});
  }
  ...
  if(['delete','clearReports','clearAll'].includes(b.action)){
    if(!env.ADMIN_PW||String(b.pw||'')!==String(env.ADMIN_PW)){
      return json({error:'관리자 비밀번호가 올바르지 않아요.'},403);
    }
    if(b.action==='delete') list=list.filter(x=>x.id!==b.id);
    else if(b.action==='clearReports'){
      const r=list.find(x=>x.id===b.id); if(r)r.rp=[];
    } else list=[];
    await save(env,list);
    return json({ok:true});
  }
