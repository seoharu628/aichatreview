...
async function token(){const a=new Uint8Array(32);crypto.getRandomValues(a);return [...a].map(x=>x.toString(16).padStart(2,'0')).join('')}
async function passHash(pass,salt){const d=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(salt+'\0'+pass));return [...new Uint8Array(d)].map(x=>x.toString(16).padStart(2,'0')).join('')}
async function getUser(env,u){const x=await env.REVIEWS.get(USER_PREFIX+u);return x?JSON.parse(x):null}
async function account(env,s){if(!s)return null;const x=await env.REVIEWS.get(SESSION_PREFIX+s);if(!x)return null;const z=JSON.parse(x);if(!z||z.exp<Date.now())return null;return getUser(env,z.user)}
...
export async function onRequestPost({ request, env }) {
  ...
  if(b.action==='accountLogin'){
    ...
    const session=await token();await env.REVIEWS.put(SESSION_PREFIX+session,JSON.stringify({user,exp:Date.now()+30*86400000}));
    return json({ok:true,user,session});
  }
  if(b.action==='accountLogout'){if(b.session)await env.REVIEWS.delete(SESSION_PREFIX+b.session);return json({ok:true});}
  ...
  if (b.action === 'add') {
    ...
    if(!user)return json({error:'회원가입 후 로그인한 사용자만 리뷰를 등록할 수 있어요'},401);
    const pid=String(b.pid||'');
    if(list.some(r=>r.uid===user.user&&r.pid===pid))return json({error:'이 계정은 이 플랫폼에 이미 리뷰를 작성했어요'},409);

    const body = String(b.body || '').trim();
    const bannedWords = ['ㅈ목','친목','친목질','친.목','ㅊ목','디씨','디씨인사이드'];
  ...
  if (b.action === 'edit') {
    ...
    const body = String(b.body || '').trim();
    const bannedWords = ['ㅈ목','친목','친.목','ㅊ목','디씨','디씨인사이드'];
    if (bannedWords.some(word => body.includes(word))) {
      return json({ error: '사용할 수 없는 단어가 포함되어 있어요' }, 400);
    }
    const rating = Math.round(Number(b.rating));
